...
- 인증 없음
- GitHub Pages 유지
- 실제 소스는 계속 로컬 Markdown 파일
  - 일반 글: `src/content/blog/*.md`
  - 42 글: `src/content/forty-two/*.md`
...
3. 상단에서 `폴더 연결` 또는 `폴더 변경`으로 레포 루트 또는 `src/content` 폴더 연결
4. `새 글 · 도구` 또는 `라이브러리`에서 새 초안 시작 / 기존 글 선택
5. 제목 → 설명 → 본문 순서로 먼저 작성
6. 필요할 때만 오른쪽 `미리보기`, `문서 설정`, `점검` 패널 열기
7. `문서 설정`에서 `공개` 또는 `비공개` 선택
8. `저장` 클릭 (저장 전 상태는 상단에 `임시저장`으로 표시)
9. 마지막으로 git 작업
   ```bash
   git add .
...
- 새 기술 글 생성
- 새 42 글 생성
- 42 글은 `/42/...` 미리보기 경로 + `src/content/forty-two/*.md` 저장
- 일반 글은 `/blog/...` 미리보기 경로 + `src/content/blog/*.md` 저장
- 기존 글 불러오기
- 수정 후 저장
- Markdown 다운로드
- 현재 글 복제
...
- 새 글과 문서 작업을 `새 글 · 도구` 메뉴로 정리
- 글 목록 / 미리보기 / 문서 설정 / 점검 패널을 필요할 때만 열기
- 라이브러리 / 오른쪽 패널은 오버레이처럼 열리고, 바깥 영역 클릭이나 `Esc`로 닫기
...
- 폴더 연결 표시에는 `blog`와 `forty-two` 두 디렉터리 상태가 함께 나타납니다.
